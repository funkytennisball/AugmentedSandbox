...
#ifndef CGAL_QT_TRIANGULATION3D_GRAPHICS_ITEM_H
#define CGAL_QT_TRIANGULATION3D_GRAPHICS_ITEM_H
...
template <typename T>
class Triangulation3DGraphicsItem : public GraphicsItem
{
  typedef typename T::Geom_traits Geom_traits;
public:
  Triangulation3DGraphicsItem(T* t_);
...
protected:
  ...
  CGAL::Qt::PainterOstream<Geom_traits> painterostream;
...
// Constructor
template <typename T>
Triangulation3DGraphicsItem<T>::Triangulation3DGraphicsItem(T * t_)
...
template <typename T>
QRectF 
Triangulation3DGraphicsItem<T>::boundingRect() const
{
...
template <typename T>
void 
Triangulation3DGraphicsItem<T>::operator()(typename T::Face_handle fh)
{
	...
	if(visible_edges) {
		for (int i=0; i<3; i++) {
			if (fh < fh->neighbor(i) || t->is_infinite(fh->neighbor(i))){
				...
				painterostream << t->segment(fh,i);		// sends to stream
			}
		}
	}
...
template <typename T>
void 
Triangulation3DGraphicsItem<T>::drawAll(QPainter *painter)
{
...
template <typename T>
void 
Triangulation3DGraphicsItem<T>::paintVertices(QPainter *painter)
{
  if(visibleVertices()) {
...
template <typename T>
void 
Triangulation3DGraphicsItem<T>::paintOneVertex(const typename T::Point& point)
{
  Converter<Geom_traits> convert;
...
template <typename T>
void 
Triangulation3DGraphicsItem<T>::paintVertex(typename T::Vertex_handle vh)
{
  Converter<Geom_traits> convert;
...
template <typename T>
void 
Triangulation3DGraphicsItem<T>::paint(QPainter *painter, 
...
template <typename T>
void 
Triangulation3DGraphicsItem<T>::updateBoundingBox()
{
...
template <typename T>
void 
Triangulation3DGraphicsItem<T>::modelChanged()
{
  if((t->number_of_vertices() == 0) ){
    this->hide();
...
#endif // CGAL_QT_TRIANGULATION3D_GRAPHICS_ITEM_H
